# Bölüm 7 İçerik Genişletmesi (Phase 18) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add full lesson content and a 290-question bank for Section 7
(topics 61-70, "Struct ve nesne yönelimli programlama"), following the
identical architecture used in Phases 13-17.

**Architecture:** One new `applySection7LessonContent` function in
`src/LessonContentSection7.cpp`, one new `appendSection7Questions`
function in `src/QuestionsSection7.cpp`, both wired into the existing
`LessonManager`/`QuestionManager` constructors after the Phase 17
calls.
...
- Ids for this phase's questions: 1523-1812 (10 topics x 29 questions
  each), continuing directly from Phase 17's last id (1522).
...
- `MultipleChoice.acceptedAnswers` is a single lowercase letter (a/b/c/d)
  ...
  execution order, running — the resulting program, not just checking
  it's a permutation) to produce the correct, intended runtime
...
- **`struct` members are `public` by default; `class` members are
  ...
  between them — both support constructors, destructors, methods,
  inheritance. Content must never imply `struct` can't have methods.
- **Unscoped `enum` values implicitly convert to `int`** and pollute
  the enclosing scope; **`enum class` values require `EnumAdi::` 
  qualification and do NOT implicitly convert to `int`** (a
  `static_cast<int>` is required).
- **If a class/struct defines ANY user-provided constructor, the
  compiler stops auto-generating the default (no-argument)
  constructor.** `ClassName nesne;` after only a parameterized
  constructor exists is a genuine compile error.
- **A class/struct has exactly one destructor** (`~ClassName()`, no
  parameters, no overloading). It runs automatically at scope exit for
  a stack object, or via `delete` for a heap object — never for a
  leaked heap object whose pointer was never `delete`d.
- **`private`** members: accessible only from the class's own member
  functions. **`public`**: accessible from anywhere. **`protected`**:
  behaves like `private` to outside code, but IS accessible from a
  derived class's own member functions. A derived class does NOT gain
  direct access to its base class's `private` members (they still
  exist, but aren't directly usable in derived-class code).
- **Construction order in inheritance: base constructor completes
  BEFORE the derived class's own constructor body runs. Destruction
  order is the exact reverse** (derived destructor runs before base
  destructor). Any question claiming a specific print order from
  constructors/destructors must be verified by actually compiling AND
  RUNNING the snippet.
...
### Task 1: Section 7 lesson content

**Files:**
- Modify: `src/LessonContent.h` (add one new declaration)
- Create: `src/LessonContentSection7.cpp`
...
- Modify: `CMakeLists.txt` (add the new `.cpp` to both the lib and test
  target sources, matching how `LessonContentSection6.cpp` is
  registered)
- Modify: `tests/LessonManagerTests.cpp` (widen two existing tests)

**Interfaces:**
- Produces: `void applySection7LessonContent(std::vector<Lesson>& lessons);`
  writing into `lessons[60]` through `lessons[69]` (topics 61-70).

- [ ] **Step 1: Add the new declaration**

In `src/LessonContent.h`, add this line after the existing
`applySection6LessonContent` declaration (do not remove that one):
...
Create `src/LessonContentSection7.cpp` with exactly this content:
...
Expected: `93/93 test(s) passed.` (same count as Phase 17's end state —
this task only renames/widens two existing tests' ranges).

- [ ] **Step 7: Commit**

```bash
git add src/LessonContent.h src/LessonContentSection7.cpp src/LessonManager.cpp CMakeLists.txt tests/LessonManagerTests.cpp
git commit -m "feat(content): add Section 7 lesson content (topics 61-70)"
```

---

### Task 2: Questions for topics 61-64 (ids 1523-1638)

**Files:**
- Modify: `src/QuestionContent.h` (add one new declaration)
- Create: `src/QuestionsSection7.cpp`
- Modify: `src/QuestionManager.cpp` (include, call, bump `reserve()`)
- Modify: `CMakeLists.txt` (add the new `.cpp`)

**Interfaces:**
- Produces: `void appendSection7Questions(std::vector<Question>& questions);`
  — Task 3 and Task 4 append more `questions.push_back(...)` calls to
  this SAME function body (in this same file) for topics 65-67 and
  68-70 respectively. This task only adds topics 61-64 (ids 1523-1638).

- [ ] **Step 1: Add the new declaration**

In `src/QuestionContent.h`, add this line after the existing
`appendSection6Questions` declaration (do not remove that one):

```cpp
void appendSection7Questions(std::vector<Question>& questions);
```

- [ ] **Step 2: Write topics 61-64's questions**

Create `src/QuestionsSection7.cpp` with exactly this content:
...
Run: `./build/Debug/CppMasterConsoleTests.exe`
Expected: `93/93 test(s) passed.` (unchanged — the test that counts
questions per topic is added in Task 4, once all of topics 61-70 have
their full 29 questions each).

- [ ] **Step 6: Commit**

```bash
git add src/QuestionContent.h src/QuestionsSection7.cpp src/QuestionManager.cpp CMakeLists.txt
git commit -m "feat(content): add questions for topics 61-64 (ids 1523-1638)"
```

---

### Task 3: Questions for topics 65-67 (ids 1639-1725)

**Files:**
- Modify: `src/QuestionsSection7.cpp` (append more `push_back` calls to
  the SAME `appendSection7Questions` function body Task 2 created —
  do not redeclare the function or touch any other file)

**Interfaces:**
- Consumes: `appendSection7Questions(std::vector<Question>& questions)`
  defined in `src/QuestionsSection7.cpp` by Task 2 — append to its body,
  after the last `questions.push_back(...)` for id 1638 and before the
  function's closing `}`.
- Produces: ids 1639-1725 added to the same function. Task 4 appends
  after this task's last id (1725).

- [ ] **Step 1: Write topics 65-67's questions**

In `src/QuestionsSection7.cpp`, inside `appendSection7Questions`, add
these `push_back` calls right after the id-1638 block (before the
function's closing `}`):
...
Run: `./build/Debug/CppMasterConsoleTests.exe`
Expected: `93/93 test(s) passed.` (unchanged — no existing test yet counts
these topics' questions).

- [ ] **Step 3: Commit**

```bash
git add src/QuestionsSection7.cpp
git commit -m "feat(content): add questions for topics 65-67 (ids 1639-1725)"
```

---

### Task 4: Questions for topics 68-70 (ids 1726-1812), test updates, and final integration

**Files:**
- Modify: `src/QuestionsSection7.cpp` (append the final `push_back`
  calls to `appendSection7Questions`)
- Modify: `tests/QuestionManagerTests.cpp` (add one new per-section
  minimum-count test)
- Modify: `docs/superpowers/roadmap.md` (mark Phase 18 complete)

**Interfaces:**
- Consumes: `appendSection7Questions(std::vector<Question>& questions)`
  in `src/QuestionsSection7.cpp` — append after Task 3's last id (1725),
  before the function's closing `}`. After this task, ids 1523-1812 are
  all present (10 topics x 29 questions) and the function is complete.

- [ ] **Step 1: Write topics 68-70's questions**

In `src/QuestionsSection7.cpp`, inside `appendSection7Questions`, add
these `push_back` calls right after the id-1725 block (before the
function's closing `}`):
...
Run: `./build/Debug/CppMasterConsoleTests.exe`
Expected: `94/94 test(s) passed.` (93 from Task 1's end state + 1 new
test).

Run from `build/`: `ctest -C Debug --output-on-failure`
Expected: 100% pass.

- [ ] **Step 4: Manual verification — topic 65 end-to-end**

Run the app (`./build/Debug/CppMasterConsole.exe`), navigate to the
topic browser, choose Section 7, choose topic 65 ("Constructor"):
...
1. Confirm the lesson text prints (matches the `explanation`/`syntax`/
   ...
   passes for topic 65 with zero `Application.cpp` changes.
...
```bash
git add src/QuestionsSection7.cpp tests/QuestionManagerTests.cpp
git commit -m "feat(content): add questions for topics 68-70 (ids 1726-1812), widen coverage tests"
```

- [ ] **Step 6: Update the roadmap**

In `docs/superpowers/roadmap.md`, add a new bullet for Phase 18 under
"Phase order," immediately after the Phase 17 entry, following the
EXACT format Phases 8-17 use
(`**Phase N — name: COMPLETE** (commits \`X..Y\`, ...)`), with the
commit range starting at this phase's spec commit (the commit for "Add
design spec for Bölüm 7 İçerik Genişletmesi (phase 18)") and ending at
this task's final content commit (from Step 5 above). Update the
"Status" section to say Phase 18 is complete and Phase 19 (Section 8
content) starts next.
...
git commit -m "docs: mark Phase 18 complete in roadmap"
```
