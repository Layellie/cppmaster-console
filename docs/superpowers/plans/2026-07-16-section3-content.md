# Bölüm 3 İçerik Genişletmesi Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Give topics 21-30 (Section 3, "Seçim ve döngüler") real lesson
content and a 29-question-per-topic question bank (290 questions total),
the second of nine content-expansion phases, following the exact same
architecture Phase 13 established and merged.

**Architecture:** Two new per-section content files
(`src/LessonContentSection3.cpp`, `src/QuestionsSection3.cpp`), declared
in the SAME shared headers Phase 13 created (`src/LessonContent.h`,
`src/QuestionContent.h` — this phase adds one more declaration to each,
it does not replace Phase 13's). No `Application.cpp` changes needed.
...
- Topics covered: 21 (switch-case), 22 (for), 23 (while), 24 (do-while),
  25 (break), 26 (continue), 27 (İç içe döngüler), 28 (Range-based for),
  29 (1'den N'e kadar toplama), 30 (Faktöriyel hesaplama) — sectionId 3.
...
- Question ids sequential starting at 363 (Phase 13 ended at 362): topic
  21 = 363-391, topic 22 = 392-420, topic 23 = 421-449, topic 24 =
  450-478, topic 25 = 479-507, topic 26 = 508-536, topic 27 = 537-565,
  topic 28 = 566-594, topic 29 = 595-623, topic 30 = 624-652. Exactly 29
  per topic.
- Per-topic type/difficulty/baseXp distribution (identical to every
  topic in Phase 13 — fixed project-wide convention):
...
- `break`/`continue` questions must correctly scope to the *innermost*
  enclosing loop only. `do-while` questions must correctly reflect that
  the body runs at least once even if the condition starts false.
  Range-based `for` questions must correctly distinguish value (`int x`)
  vs. reference (`int& x`) iteration semantics.

---

### Task 1: Section 3 lesson content

**Files:**
- Modify: `src/LessonContent.h` (add one new declaration)
- Create: `src/LessonContentSection3.cpp`
...
**Interfaces:**
- Produces: `void applySection3LessonContent(std::vector<Lesson>& lessons);`
  — writes into indices 20-29 (topics 21-30).

- [ ] **Step 1: Add the new declaration**

In `src/LessonContent.h`, add this line after the existing
`applySection2LessonContent` declaration (do not remove that one):
...
In `tests/LessonManagerTests.cpp`, change the Phase-13 test names/ranges.
Replace:
...
Expected: `89/89 test(s) passed.` (same count as Phase 13's end state —
this task only renames/widens two existing tests' ranges, it does not
add or remove any test case).

- [ ] **Step 7: Commit**

```bash
git add src/LessonContent.h src/LessonContentSection3.cpp src/LessonManager.cpp CMakeLists.txt tests/LessonManagerTests.cpp
git commit -m "feat(content): add Section 3 lesson content (topics 21-30)"
```

---

### Task 2: Questions for topics 21-24 (ids 363-478)

**Files:**
- Modify: `src/QuestionContent.h` (add one new declaration)
- Create: `src/QuestionsSection3.cpp`
- Modify: `src/QuestionManager.cpp` (include, call, bump `reserve()`)
- Modify: `CMakeLists.txt` (add the new `.cpp`)

**Interfaces:**
- Produces: `void appendSection3Questions(std::vector<Question>& questions);`
  — Task 3 and Task 4 append more `questions.push_back(...)` calls to
  this SAME function body (in this same file) for topics 25-27 and
  28-30 respectively. This task only adds topics 21-24 (ids 363-478).

- [ ] **Step 1: Add the new declaration**

In `src/QuestionContent.h`, add this line after the existing
`appendSection2Questions` declaration (do not remove that one):

```cpp
void appendSection3Questions(std::vector<Question>& questions);
```

- [ ] **Step 2: Write topics 21-22's questions**
...
Run: `./build/Debug/CppMasterConsoleTests.exe`
Expected: `89/89 test(s) passed.` (topics 21-24 now have 116 more
...
```bash
git add src/QuestionContent.h src/QuestionsSection3.cpp src/QuestionManager.cpp CMakeLists.txt
git commit -m "feat(content): add questions for topics 21-24 (ids 363-478)"
```

---

### Task 3: Questions for topics 25-27 (ids 479-565)

**Files:**
- Modify: `src/QuestionsSection3.cpp` (append more
  `questions.push_back(...)` calls to the existing `appendSection3Questions`
  function body, right before its closing `}`)

**Interfaces:**
- Consumes: `appendSection3Questions`'s existing body from Task 2 (ends
  after topic 24's WriteCode question, id 478).
- Produces: the same function extended through id 565. Task 4 appends
  topics 28-30 after this task's content, same file, same function.

- [ ] **Step 1: Append topics 25-27's questions**

In `src/QuestionsSection3.cpp`, insert the following right before the
function's closing `}` (i.e. immediately after the `id = 478` WriteCode
...
Run: `./build/Debug/CppMasterConsoleTests.exe`
Expected: `89/89 test(s) passed.` (unchanged — no existing test yet counts
these topics' questions).

- [ ] **Step 3: Commit**

```bash
git add src/QuestionsSection3.cpp
git commit -m "feat(content): add questions for topics 25-27 (ids 479-565)"
```

---

### Task 4: Questions for topics 28-30 (ids 566-652), test updates, and final integration

**Files:**
- Modify: `src/QuestionsSection3.cpp` (append the final
...
- Modify: `tests/QuestionManagerTests.cpp` (add a per-topic minimum-count
  test for topics 21-30)
- Modify: `docs/superpowers/roadmap.md` (mark Phase 14 complete)

**Interfaces:**
- Consumes: `appendSection3Questions`'s body from Tasks 2-3 (ends after
  topic 27's WriteCode question, id 565).
- Produces: the complete, final `appendSection3Questions` function
  (ids 363-652, 290 questions across topics 21-30).

- [ ] **Step 1: Append topics 28-30's questions**

In `src/QuestionsSection3.cpp`, insert the following right before the
function's closing `}` (i.e. immediately after the `id = 565` WriteCode
...
`QuestionManager_NoDuplicateQuestionIds`, which Phase 13 already widened
to scan topics 1-100):
...
Run: `./build/Debug/CppMasterConsoleTests.exe`
Expected: `90/90 test(s) passed.` (89 from Phase 13's end state + 1 new
test).

Run from `build/`: `ctest -C Debug --output-on-failure`
Expected: 100% pass.

- [ ] **Step 4: Manual verification — topic 22 end-to-end**

Run `./build/Debug/CppMasterConsole.exe`, navigate to the topic browser,
choose Section 3, choose topic 22 ("for"):
...
1. Confirm the lesson text prints (matches the `explanation`/`syntax`/
   ...
   passes for topic 22 with zero `Application.cpp` changes.
2. Confirm "Konu testi başlıyor (29 soru)." prints — proving all 29 of
   topic 22's questions are wired into `questionsForTopic(22)`.
3. Answer a few questions (correct or incorrect, either is fine — this
   step confirms the flow works, not re-verifying Phase 12's already-
   tested adaptive logic), then answer `cikis` to return to the main
   menu.
4. Confirm the app returns to the main menu without crashing, and that
   partial XP is awarded.
5. Clean up: `rm -f data/*.txt` (remove any progress/mistake files
   created during this manual run).

- [ ] **Step 5: Commit the question content and test changes**

```bash
git add src/QuestionsSection3.cpp tests/QuestionManagerTests.cpp
git commit -m "feat(content): add questions for topics 28-30 (ids 566-652), widen coverage tests"
```

- [ ] **Step 6: Update the roadmap**

In `docs/superpowers/roadmap.md`, add a new bullet for Phase 14 under
"Phase order," immediately after the Phase 13 entry, following the
EXACT format Phases 8-13 use
(`**Phase N — name: COMPLETE** (commits \`X..Y\`, ...)`), with the
commit range starting at this phase's PLAN commit (not the spec commit —
see Phase 13's own final-review-caught mistake on this exact point) and
ending at this task's final content commit (from Step 5 above). Update
the "Status" section to say Phase 14 is complete and Phase 15 (Section 4
content) starts next.
...
git commit -m "docs: mark Phase 14 complete in roadmap"
```
